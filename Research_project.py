...
#import datasets
from sklearn.datasets import load_files
reviews_train = load_files("C:/hmm/courses/3rd semester/Special topics/Research Project/aclImdb_v1/aclImdb/train/")
text_train, y_train = reviews_train.data[:100], reviews_train.target[:100]

print("Number of documents in train data: {}".format(len(text_train)))
print("Samples per class (train): {}".format(np.bincount(y_train)))

reviews_test = load_files("C:/hmm/courses/3rd semester/Special topics/Research Project/aclImdb_v1/aclImdb/test/")
text_test, y_test = reviews_test.data[:100], reviews_test.target[:100]
...
param_grid = {'C': [0.001, 0.01, 0.1, 1, 10, 5, 2, .5]}
...
new_model = LogisticRegression(C=grid.best_params_['C'], max_iter=10000)
new_model.fit(X_train, y_train)

# lr.fit(X_train, y_train)
new_model.predict(X_test)
print("Score: {:.2f}".format(new_model.score(X_test, y_test)))
...
print("Pos prediction: {}". format(new_model.predict(vect.transform(pos))))
...
print("Neg prediction: {}". format(new_model.predict(vect.transform(neg))))
